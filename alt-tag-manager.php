<?php
/**
 * Plugin Name: PhilDesigns Alt Tag Manager
...
class Search_Alt_Tags {
	...
	public function plugin_action_links( $links ) {
		$plugin_links = array(
			'<a href="' . admin_url( 'upload.php?page=search-alt-tags-settings' ) . '">' . __( 'Settings', 'alt-tag-manager' ) . '</a>',
			'<a href="' . admin_url( 'upload.php?page=search-alt-tags' ) . '">' . __( 'Scan', 'alt-tag-manager' ) . '</a>',
		);
	...
	public function register_menus() {
		add_media_page(
			__( 'Search Alt Tags', 'alt-tag-manager' ),
			__( 'Search Alt Tags', 'alt-tag-manager' ),
			'upload_files',
			'search-alt-tags',
		...
		add_submenu_page(
			'upload.php',
			__( 'Alt Tags — Settings', 'alt-tag-manager' ),
			__( 'Alt Tag Settings', 'alt-tag-manager' ),
			'manage_options',
	...
	public function enqueue_assets( $hook ) {
		...
		wp_localize_script( 'sat-admin', 'SAT', array(
			...
			'i18n'            => array(
				'saving'       => __( 'Saving…', 'alt-tag-manager' ),
				'saved'        => __( 'Saved!', 'alt-tag-manager' ),
				'generating'   => __( 'Generating…', 'alt-tag-manager' ),
				'scanning'     => __( 'Scanning theme files…', 'alt-tag-manager' ),
				'error'        => __( 'Error. Try again.', 'alt-tag-manager' ),
				'confirmBulk'  => __( 'Generate AI alt tags for all images shown? Each image will be analysed by Claude and saved automatically.', 'alt-tag-manager' ),
				'confirmRescan'=> __( 'Rescan both the media library and theme templates now?', 'alt-tag-manager' ),
				'noIssues'     => __( 'No issues found — everything looks good!', 'alt-tag-manager' ),
				/* translators: %1$d: current item number, %2$d: total count */
				'bulkProgress' => __( 'Processing %1$d of %2$d…', 'alt-tag-manager' ),
				/* translators: %d: number of alt tags generated */
				'bulkDone'     => __( 'Done! Generated %d alt tags.', 'alt-tag-manager' ),
			),
		) );
	}
	...
	public function ajax_get_all_media_ids() {
		check_ajax_referer( 'sat_nonce', 'nonce' );
		if ( ! current_user_can( 'upload_files' ) ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient permissions.', 'alt-tag-manager' ) ) );
		}
	...
	public function ajax_save_alt_tag() {
		...
		if ( ! $attachment_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid attachment ID.', 'alt-tag-manager' ) ) );
		}
		...
		wp_send_json_success( array(
			'message'       => __( 'Alt tag saved.', 'alt-tag-manager' ),
	...
	public function ajax_generate_alt_tag() {
		...
		$api_key = get_option( 'sat_anthropic_api_key', '' );
		if ( empty( $api_key ) ) {
			wp_send_json_error( array( 'message' => __( 'Anthropic API key not configured. Visit Alt Tag Settings.', 'alt-tag-manager' ) ) );
		}

		$image_url = wp_get_attachment_url( $attachment_id );
		if ( ! $image_url ) {
			wp_send_json_error( array( 'message' => __( 'Could not get image URL.', 'alt-tag-manager' ) ) );
		}
	...
	public function ajax_save_settings() {
		...
		wp_send_json_success( array( 'message' => __( 'Settings saved.', 'alt-tag-manager' ) ) );
	}
	// -------------------------------------------------------------------------
	...
	public function ajax_ignore_theme_issue() {
		...
		if ( empty( $key ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid key.', 'alt-tag-manager' ) ) );
		}
	...
	public function ajax_import_media_csv() {
		...
		if ( empty( $_FILES['csv_file'] ) || ! isset( $_FILES['csv_file']['error'] ) || UPLOAD_ERR_OK !== (int) $_FILES['csv_file']['error'] ) {
			wp_send_json_error( array( 'message' => __( 'No file uploaded or upload error.', 'alt-tag-manager' ) ) );
		}
		...
		if ( 'csv' !== $ext ) {
			wp_send_json_error( array( 'message' => __( 'Please upload a .csv file.', 'alt-tag-manager' ) ) );
		}

		$handle = fopen( $file['tmp_name'], 'r' ); // phpcs:ignore WordPress.WP.AlternativeFunctions
		if ( ! $handle ) {
			wp_send_json_error( array( 'message' => __( 'Could not read the uploaded file.', 'alt-tag-manager' ) ) );
		}
		...
		if ( ! $headers ) {
			...
			wp_send_json_error( array( 'message' => __( 'CSV file is empty or invalid.', 'alt-tag-manager' ) ) );
		}
		...
		if ( false === $id_col || false === $alt_col ) {
			fclose( $handle ); // phpcs:ignore WordPress.WP.AlternativeFunctions
			wp_send_json_error( array( 'message' => __( 'CSV must contain "ID" and "Alt Tag" columns. Download the template CSV for the correct format.', 'alt-tag-manager' ) ) );
		}
